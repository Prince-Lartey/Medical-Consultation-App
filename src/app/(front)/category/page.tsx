...
export default async function Page({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
    ...
    const doctors = allDoctors.filter((doctor: Doctor) => doctor.doctorProfile?.operationMode === mode);

    const services = (await getServices()).data || []

    return (
        <div className="container p-8">
            <h1 className="capitalize scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-4xl pb-6">{mode} ({doctors.length.toString().padStart(2, "0")})</h1>
