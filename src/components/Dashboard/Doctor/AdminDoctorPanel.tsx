    ...
    return (
        <ScrollArea className="h-[32rem] w-full">
            <div className="p-4">
                {doctors.length > 0 ? ( 
                    doctors.map((doctor) => (
                        <Link key={doctor.id}  href={`/dashboard/doctors/view/${doctor.id}`} className={cn("border mb-2 border-gray-300 shadow-sm text-xs py-3 px-2 inline-block w-full bg-white dark:text-slate-900 rounded-md", pathname === `/dashboard/doctors/view/${doctor.id}` && "border-gray-700 border-2 dark:border-blue-500 bg-gray-100")}>
                            <div className='flex justify-between items-center pb-2'>
                                <div className="flex items-center">
                                    <Avatar className="h-9 w-9 sm:flex mr-2">
                                ...
                                <div className={cn("flex items-center", doctor.doctorProfile?.status === "approved" ? "text-green-500" : doctor.doctorProfile?.status === "rejected" ? "text-red-500" : "text-yellow-500")}>
                                    {
