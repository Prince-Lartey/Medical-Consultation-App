        ...
        <div className="grid lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 grid-col-1 gap-6">
            {
                specialties?.map((specialty, index) => {
                    return (
                        <Link key={index} href={`/specialty/${specialty.slug}`} className={`rounded-md py-3 px-6 flex gap-4 bg-slate-800 text-slate-50 justify-between items-center ${className}`}>
